...
def _mask_to_img(mask_file):
    img_file = re.sub('/home/skeri/tentorch/testing_folder/4M/masks'.format(IMG_DIR),
                      '/home/skeri/tentorch/testing_folder/4M/orig'.format(IMG_DIR), mask_file)
    img_file = re.sub('\.png$', '.jpg', img_file)
    return img_file


def _img_to_mask(img_file):
    mask_file = re.sub('/home/skeri/tentorch/testing_folder/4M/orig'.format(IMG_DIR),
                       '/home/skeri/tentorch/testing_folder/4M/masks'.format(IMG_DIR), img_file)
    mask_file = re.sub('\.jpg$', '.png', mask_file)
    return mask_file


def get_img_files():
    mask_files = sorted(glob('/home/skeri/tentorch/testing_folder/4M/masks/*.png'.format(IMG_DIR)))
    return np.array([_mask_to_img(f) for f in mask_files])
